        ...
        <div style="width: 100%;">
            ...
            foreach ($result0 as $cle => $valeur) {
                ...
                if ($is_locked_clean == 0) {
                    ...
                    ?>
                    <hr>
                    <p> Nom de l'annonce : <?php echo $ann_name_clean ?> | Nombre de vues :  <?php echo $ann_views_clean ?> |
                    Nom d'utilisateur de l'annonceur : <?php echo $ann_username_clean ?> </p>
                    <p> Description : <?php echo $ann_text_clean ?> </p>
...
if (isset($_POST['acc_ann'])) {
                    ...
                    $result0 = $ann_lock_time1->fetchAll();

                    $ann_id1 = $pdo->prepare("SELECT ann_id FROM annonces");
                    $ann_id1->execute();
                    $result1 = $ann_id1->fetchAll();

                    $ann_name1 = $pdo->prepare("SELECT ann_name FROM annonces");
                    $ann_name1->execute();
                    $result2 = $ann_name1->fetchAll();

                    foreach ($result0 as $cle => $valeur) {

                    $ann_lock_time_cut1 = json_encode(array_slice($result0, $cle, $valeur));
                    $str5 = $ann_lock_time_cut1;
                    $order5 = array("[", "{", "ann_lock_time", ":", "}", "]", '"', ',');
                    $replace5 = '';
                    $ann_lock_time_clean1 = str_replace($order5, $replace5, $str5);

                    $ann_id_cut1 = json_encode(array_slice($result1, $cle, $valeur));
                    $str6 = $ann_id_cut1;
                    $order6 = array("[", "{", "ann_id", ":", "}", "]", '"', ',');
                    $replace6 = '';
                    $ann_id_clean1 = str_replace($order6, $replace6, $str6);

                    $ann_name_cut1 = json_encode(array_slice($result2, $cle, $valeur));
                    $str7 = $ann_name_cut1;
                    $order7 = array("[", "{", "ann_name", ":", "}", "]", '"', ',');
                    $replace7 = '';
                    $ann_name_clean1 = str_replace($order7, $replace7, $str7);
